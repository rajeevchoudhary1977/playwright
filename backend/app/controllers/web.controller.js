...
export const createTest = async (req, res) => {
  ...
  switch (body.category) {
    case TEST_CATEGORIES.REGRESSION_TEST: {
      const { testTimeout, jsonCode, playwrightConfigTsCode, regressionPageByPage: regressionPageByPageString, ...testBody } = body;
      
      const regressionPageByPage = regressionPageByPageString ? JSON.parse(regressionPageByPageString) : false;

      testBody.mainUrl = testBody.mainUrl ? (regressionPageByPage ? testBody.mainUrl.split(",") : testBody.mainUrl) : null;
      testBody.refUrl = testBody.refUrl ? (regressionPageByPage ? testBody.refUrl.split(",") : testBody.refUrl) : null;
      ...
      if (test) {
        ...
        const jsonToUpdate = { regressionPageByPage: regressionPageByPage, mainUrl: test.mainUrl, testTimeout: parseInt(testTimeout) };
    ...
    case TEST_CATEGORIES.VISUAL_TEST: {
      const {
        jsonCode,
        /* playwrightConfigTsCode,*/ pageByPage,
        pageByPageUrls,
        refImgs,
        isMobile,
        selectedDevice,
    ...
    case TEST_CATEGORIES.MLR_PKG: {
      const { testTimeout, jsonCode, playwrightConfigTsCode, mlrPageByPage: mlrPageByPageString, ...testBody } = body;

      const mlrPageByPage = mlrPageByPageString ? JSON.parse(mlrPageByPageString) : false;

      testBody.mainUrl = testBody.mainUrl ? (mlrPageByPage ? testBody.mainUrl.split(",") : testBody.mainUrl) : null;
      testBody.refUrl = null;
      ...
      if (test) {
        ...
        const jsonToUpdate = { mlrPageByPage: mlrPageByPage, mainUrl: test.mainUrl, testTimeout: parseInt(testTimeout) };
        if (test.refUrl) jsonToUpdate.refUrl = test.refUrl;
...
export const getTestConfigJsonCode = async (req, res) => {
  ...
  const configJsonPath = path.join(__dirname, "../../public/web_apps", webApp.engagement, webApp.name, test.category, test.name, "config.json");
...
export const getTestBrowserJSON = async (req, res) => {
  ...
  const browserJSONPath = path.join(__dirname, "../../public/web_apps", webApp.engagement, webApp.name, test.category, test.name, "browser.json");
...
export const getTestUploadedImages = async (req, res) => {
  ...
  const imagesPath = path.join(__dirname, "../../public/web_apps", webApp.engagement, webApp.name, test.category, test.name, "ref-images");
...
export const renameWebApp = async (req, res) => {
  ...
  const engagementPath = path.join(__dirname, "../../public/web_apps", webApp.engagement);
...
export const updateWebAppEngagement = async (req, res) => {
  ...
  const oldWebAppPath = path.join(__dirname, "../../public/web_apps", oldEngagement, webAppName);

  const newWebAppPath = path.join(__dirname, "../../public/web_apps", newEngagement, webAppName);
